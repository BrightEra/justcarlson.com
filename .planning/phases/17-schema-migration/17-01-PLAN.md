...
must_haves:
  truths:
    - "Running migrate-schema.sh converts status: Published posts to draft: false"
    - "Running migrate-schema.sh sets draft: true for non-published posts"
...
1. **Create backup:** `cp "$file" "${file}.bak"` before any modification

2. **Determine draft value:**
   - Use yq to check if status array contains "Published": `yq --front-matter=extract '.status // [] | contains(["Published"])' "$file"`
   - If contains "Published" -> set draft: false
   - Otherwise -> set draft: true (including empty/missing status)

3. **Preserve or backfill pubDatetime:**
   - Check if pubDatetime exists and is not empty: `yq --front-matter=extract '.pubDatetime // ""' "$file"`
   - If pubDatetime empty AND draft will be false: backfill with file mtime using `date -r "$file" -Iseconds`
   - If pubDatetime exists: preserve it (do nothing)

4. **Update frontmatter using yq:**
   - Set draft field: `yq --front-matter=process -i '.draft = false' "$file"` (or true)
...
1. **Dry-run first:** `./scripts/migrate-schema.sh --dry-run`
   ...
   - Hello World should become draft: false (has empty status but was published via sync)
   - Others should become draft: true
