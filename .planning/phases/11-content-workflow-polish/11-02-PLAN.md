...
Purpose: Improve skill discoverability with consistent naming, provide smart startup suggestions
...
<task type="auto">
  <name>Task 1: Rename skill directories and update names</name>
  ...
  <action>
Rename each skill directory and update the frontmatter name field:

1. `install` -> `blog-install`
...
2. `publish` -> `blog-publish`
   - Update `name: publish` to `name: blog-publish`
   - Update description to: "Publish blog posts from Obsidian vault with oversight"

3. `list-posts` -> `blog-list-posts`
   - Update `name: list-posts` to `name: blog-list-posts`
   - Keep existing description

4. `maintain` -> `blog-maintain`
   - Update `name: maintain` to `name: blog-maintain`
   - Keep existing description

5. `unpublish` -> `blog-unpublish`
   - Update `name: unpublish` to `name: blog-unpublish`
   - Keep existing description

Use git mv to rename directories to preserve history:
```bash
cd .claude/skills
git mv install blog-install
git mv publish blog-publish
git mv list-posts blog-list-posts
git mv maintain blog-maintain
git mv unpublish blog-unpublish
```

Then update each SKILL.md file's name field.

IMPORTANT: Do NOT use colons in skill names - they are not valid. Use hyphens.
  </action>
  <verify>
1. `ls .claude/skills/` shows all blog-* directories
2. `grep "name: blog-" .claude/skills/*/SKILL.md` shows all 5 skills have correct names
3. Old directories no longer exist
  </verify>
  <done>All 5 skills renamed to blog-* prefix with updated frontmatter</done>
</task>
...
<verification>
1. Skill discovery:
   - `ls .claude/skills/` shows: blog-help, blog-install, blog-list-posts, blog-maintain, blog-publish, blog-unpublish

2. Name validation:
   - `grep -h "^name:" .claude/skills/*/SKILL.md | sort` shows all 6 blog-* names
