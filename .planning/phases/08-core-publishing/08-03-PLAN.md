...
must_haves:
  truths:
    - "Biome lint runs before any commit"
    - "Full build (`npm run build`) runs before any push"
    - "Lint/build failures block commit and push"
...
3. **Verification pipeline**:
   - After all posts copied (before any git operations):
     a. Run lint
     b. If lint fails: attempt fix (see retry logic)
     c. Run build
     d. If build fails: attempt fix (see retry logic)
   - Both must pass before proceeding to commits
...
<verification>
1. `npm run lint` runs before commit
2. `npm run build` runs before push
3. Failures block git operations
